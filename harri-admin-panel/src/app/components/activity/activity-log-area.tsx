    ...
    <div className="rounded-md bg-white p-4 sm:p-6 shadow-xs">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mb-0 text-xs text-gray6">Sistem akışları, sipariş olayları ve güvenlik kontrolleri.</p>
        </div>
        <div className="flex items-center gap-2">
          <label htmlFor="eventType" className="text-xs text-gray6">
            Event:
          </label>
          <select
            id="eventType"
      ...
      {logs.length === 0 ? (
        <p className="rounded-md border border-dashed border-gray6 p-4 text-sm text-gray6">Henüz log kaydı bulunmuyor.</p>
      ) : (
          ...
          {logs.map((log) => {
            ...
            return (
              <article key={log.id} className="rounded-md border border-gray6 p-3 sm:p-4">
                ...
                <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-gray6">
                  <span>Actor: {log.actor || "system"}</span>
                  <span>Target: {log.targetType || "-"} {log.targetId || ""}</span>
                </div>
                {log.metadata && (
                  <pre className="mt-2 overflow-x-auto rounded bg-slate-50 p-2 text-[11px] text-slate-600">
