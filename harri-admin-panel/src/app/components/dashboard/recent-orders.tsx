...
const RecentOrders = () => {
  ...
  if (!isLoading && isError) {
    content = <ErrorMsg msg="Sipariş verileri alınırken bir hata oluştu." />;
  }
              ...
              <article key={order._id} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                <div className="mb-3 flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="mb-1 text-xs text-slate-500">Fatura</p>
                ...
                <div className="mb-3 grid grid-cols-2 gap-2 text-xs">
                  <div className="rounded-md bg-slate-50 p-2">
                    <p className="mb-1 text-slate-500">Müşteri</p>
                    <p className="mb-0 truncate font-medium text-slate-900">{order.name || "-"}</p>
                  </div>
                  <div className="rounded-md bg-slate-50 p-2">
                    <p className="mb-1 text-slate-500">Tutar</p>
        ...
        <div className="px-4 pt-6 border-t border-gray6">
          <div className="pagination flex flex-col justify-between sm:flex-row">
            <span className="flex items-center uppercase">
               1-{currentItems.length} / {orders.length} sipariş gösteriliyor
            </span>
            <Pagination
